// ! File Path: /Rock-of-Ages-Python/CLIENT-Rock_of_Ages_NSS-Python-Django/src/components/RockList.jsx

import React, { useEffect } from 'react';

export const RockList = ({ showAll, fetchRocks, rocks }) => {
    useEffect(() => {
        fetchRocks(showAll);
    }, [showAll, fetchRocks]);
        ...
        if (rocks && rocks.length) {
            return rocks.map(rock => (
                <div key={`key-${rock.id}`} className="border p-5 border-solid hover:bg-fuchsia-500 hover:text-violet-50 rounded-md border-violet-900 mt-5 bg-slate-50">
                    <div>{rock.name} ({rock.type ? rock.type.label : 'Unknown Type'}) weighs: {rock.weight}kg</div>  
                    <div>Rock owner: {rock.user ? `${rock.user.first_name} ${rock.user.last_name}` : 'Unknown Owner'}</div>  
                    {
                        showAll
                            ? ""
                            : <div>
